...
class ProductManager {
    ...
    async addProduct({ id, title, description, price, img, code, stock }) {
        try {
            ...
            const existeProduct = await ProductModel.findeOne({code: code})
    ...
    async updateProduct(id, updatedFields) {
        try {
            const arrayProducts = await ProductManager.findByIdAnUpdate(id, updatedFields)
    ...
    async deleteProduct(id) {
        try {
            const arrayProducts = await ProductModel.findyByIdAndDelete(id)
            const index = arrayProducts.findIndex(item => item.id === id)
